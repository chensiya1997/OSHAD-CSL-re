...
def get_parameters():
    ...
    parser.add_argument('--seed', type=int, default=40, help='set the random seed for stabilizing experiment results')
    parser.add_argument('--dataset', type=str, default='KPI', choices=['KPI'])
    ...
    parser.add_argument('--graph_conv_type', type=str, default='cheb_graph_conv', choices=['cheb_graph_conv'])
...
def test(zscore, loss, model, test_iter, args):
    ...
    for x, y in test_iter:
        y_pred = model(x).view(len(x), -1)
        if count==0:
            real_value=pd.DataFrame(y)
            predict_value=pd.DataFrame(y_pred)
        else:
            real_value=real_value.append(pd.DataFrame(y))
            predict_value=predict_value.append(pd.DataFrame(y_pred))
        count=count+1
    pd.DataFrame(predict_value).to_csv("/data/KPI_predictvalue.csv",index=False)
    pd.DataFrame(real_value).to_csv("/data/KPI_real.csv",index=False)
